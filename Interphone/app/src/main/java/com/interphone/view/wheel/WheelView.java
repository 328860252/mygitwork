...
public class WheelView extends View {
	...
	private static final int VALUE_OFFSET_X =20;
	
	/** Label offset Y轴偏移标签偏移
	 * -往上 +往下 */
	private static final int LABEL_OFFSET_Y = -25;
	...
	private void initResourcesIfNecessary() {
		...
		if (centerDrawable == null) {
			if(centerDrawableSourceId>0) {
				centerDrawable = getContext().getResources().getDrawable(centerDrawableSourceId);
			} else {
				centerDrawable = getContext().getResources().getDrawable(R.drawable.bg_null);
			}
		}
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		int width = widthSize;
		int maxLength = getMaxTextLength()+1;
		if (maxLength > 0) {
		...
		if (widthLabel > 0) {
			if (labelLayout == null || labelLayout.getWidth() > widthLabel) {
				...
				labelLayout = new StaticLayout(label, valuePaint,
						widthLabel, Layout.Alignment.ALIGN_NORMAL, 1.0f,
	...
	private void drawValue(Canvas canvas) {
		...
		if (labelLayout != null) {
			...
			canvas.translate(VALUE_OFFSET_X+itemsLayout.getWidth() + LABEL_OFFSET, bounds.top +LABEL_OFFSET_Y );
		...
		if (valueLayout != null) {
			canvas.save();
			//-5是因为中间的 字体 ，变大， 所以要往上挪一点
				canvas.translate(0+VALUE_OFFSET_X, bounds.top + scrollingOffset);
	...
	private void drawItems(Canvas canvas) {
		...
		//canvas.translate(0+VALUE_OFFSET_X +getWidth()/2-itemsWidth/2, - top + scrollingOffset);
		canvas.translate(0+VALUE_OFFSET_X, - top + scrollingOffset);
	...
	private int centerDrawableSourceId;

	public void setCenterDrawableSourceId(int id) {
		this.centerDrawableSourceId = id;
	}
